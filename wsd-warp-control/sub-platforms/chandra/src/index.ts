...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext  async fetch(request: Request, env: Env, ctx: Exquest.url);
    const pane = "chandra";
    const domain = "chandra.wsdenterprisesworldwide.com";

    if (url.pathname === "/api/status") {
      const laneInfo = await env.CONFIG.get("lane:WSD_TOTAL_INFRASTRUCTURE") || "      const   return Response.json({ pane, status: "operational", domain, lane: laneInfo, timestamp: new Date().toISOString() });
    }

    return Response.json({ pane, status: "operational",     return Response.json(
  cat > sub-platforms/chandra/dist/index.html << PFEOF
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Chandra â€” WSD Enterprise Sub-Platform</title>
  <style>
